...
这里建议大家把磁头的位置和磁带的内容都放到Tape类中进行处理，Executor中只维护一个TM对象和磁带信息，在每次执行后都调用一些更新的方法，这样模块性会好一些。一定要清楚哪些内容应该内聚，哪些内容应该解耦，

比如TM对象拥有当前状态和转移函数的信息，那么判断是否进入终止态或者给出当前需要的迁移函数的工作就应该放在TM对象中完成
