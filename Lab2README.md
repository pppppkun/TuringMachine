...
execute()方法需要让读取的图灵机在事先给好的磁带上执行。我们在测试的时候，会先构造好一个图灵机对象，然后将对象和磁带作为参数传给Executor类，并且执行execute方法，execute方法相当于调试时的单步执行，这样是为了方便我们测试大家的实现。snapShot方法需要能够支持快照功能，即按照指定的格式，返回以下信息

- 图灵机的状态
- 执行的步数
- 每一条磁带上的情况
  - 包括每个磁道和磁头，磁头需要指示出现在位于哪个字符下面
  - 每个磁道打印出最左非空格符号道最右非空格符号的内容
...
本次实验中，我们对图灵机的格式做一个小小的更新，加入一个`#T`来表示每条磁带上磁道的数量，他的格式如下
...
这表示第一个磁带有一个磁道，第二个磁带上有一个磁道，第三个磁带上有两个磁道，最后一个磁带上有四个磁道，其他情况下类似的解释。
...
同理，如果是两个磁道的话（本例与上一个例子的输入和图灵机无关），需要打印出如下信息
...
需要注意的是冒号和前面的label之间的关系，**冒号总是紧接着最长的label后面**，冒号后如果有内容，一定会有一个空格在冒号和内容之间，如
...
当然，得益于Java本身自带了很多库，在不改变原有代码结构的情况下也可以做到O(1)的查询速度。因为大家只需要实现一个方法，所以这次作业自定义的程度比较高，大家可以根据自己的想法实现。如果大家想要自己构思一个结构的话，建议大家遵守面向对象设计原则，把高内聚低耦合放在心上，做好数据和职责的一致性。
...
因为图灵机磁带和磁道的长度都是无限的，在输入的两端都是无限的空格，但是我们显然无法给出一个无限长度的输入，因此，大家需要自己做好磁带磁道的变长工作。

大部分需要检查的不合法的地方都是和集合与集合之间的关系有关的，幸好Java已经提供好了相关的库可以让我们调用，求两个Set的交集可以用`retainAll`这个方法，并集、差集大家可以自己去研究一下。

错误处理相关的信息在加载图灵机的时候就可以检测出一部分了，如果大家实验一写的代码可拓展性比较好的话，很容易继续往里面添加内容。实验二的代码在实验三中还会继续用到，希望大家保持一个良好的代码风格。
