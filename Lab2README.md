...
execute()方法需要让读取的图灵机在事先给好的磁带上执行。我们在测试的时候，会先构造好一个图灵机对象，然后将对象和磁带作为参数传给Executor类，并且执行execute方法。snapShot方法需要能够支持快照功能，即按照指定的格式，返回以下信息
...
我们的输入对磁带和磁道的数量没有限制，因为我们是通过ArrayList传入的数据，所以大家可以很容易的获取到磁道的具体数量，实际上这也没有对编程增加难度。
